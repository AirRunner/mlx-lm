...
class QuantizedKVCache(_BaseCache):
    ...
    def update_and_fetch(self, keys, values):
        ...
        keys = mx.quantize(keys, group_size=self.key_group_size, bits=self.key_bits)
        values = mx.quantize(
...
class QuantizedRotatingKVCache(RotatingKVCache):
    ...
    def _trim(self, trim_size, v, append=None):
        if self._is_quantized(v):
            ...
            if len(parts) == 1:
                return parts[0]
            result = tree_map(lambda *arrs: mx.concatenate(arrs, axis=2), *parts)
            return result
    ...
    def _update_concat(self, keys, values):
        ...
        q_values = list(
            mx.quantize(values, group_size=self.value_group_size, bits=self.value_bits)
        )
    ...
    def _update_in_place(self, keys, values):
        ...
        if self.keys is None or (
            ...
            if self.keys is not None:
                ...
                new_v = _init_quant(v_head_dim, self.value_bits, self.value_group_size)
                self.keys = tree_map(
                ...
                self.keys = _init_quant(k_head_dim, self.key_bits, self.key_group_size)
        ...
        # Quantize and assign
        q_keys = mx.quantize(keys, group_size=self.key_group_size, bits=self.key_bits)
